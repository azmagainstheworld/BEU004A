...
function TrashGlobal() {
  ...
  const tableRef = useRef(null); // Ref untuk menyimpan instance DataTable

  const availableModules = ["Semua", ...Object.keys(MODULE_CONFIG)];

  // 1. Verifikasi Role (Cukup Sekali)
  useEffect(() => {
    const token = localStorage.getItem("token");
    if (token) {
      try {
  ...
  // 2. Fetch Data (Stabil dengan Dependensi Kosong)
  const fetchAllTrash = useCallback(async () => {
    const token = localStorage.getItem("token");
    if (!token) return;
    ...
    let combinedData = [];
    const promises = Object.entries(MODULE_CONFIG).map(([modulName, config]) => 
      fetch(config.fetch, { headers })
        .then((res) => res.json())
        .then((data) => {
          if (Array.isArray(data)) {
            return data.map((item) => {
              let rawNominalValue = 0;
              if (modulName === "Outgoing") rawNominalValue = item.nominal_bersih;
              else if (modulName === "Pengeluaran") rawNominalValue = item.nominal_pengeluaran;
              else if (modulName !== "Karyawan" && modulName !== "User") rawNominalValue = item.nominal;
              
              return {
                ...item,
                id_unik: item[config.idKey],
                modul_sumber: modulName,
                nominal: Number(rawNominalValue) || 0,
                tanggal: item[config.dateKey] || null,
                detail_tambahan: modulName === "Karyawan" ? item.nama_karyawan : (modulName === "User" ? item.username : (item.keterangan || item.nama_pengeluaran || "-")),
              };
            });
          }
          return [];
    ...
    const results = await Promise.all(promises);
    results.forEach((data) => { combinedData = [...combinedData, ...data]; });
    setTrashData(combinedData);
  }, []);
  ...
  // 3. DataTable Manager (Pencegah Layar Putih)
  useEffect(() => {
    const tableId = "#trashTable";

    // Hancurkan tabel jika sudah ada sebelum inisialisasi ulang
    if ($.fn.DataTable.isDataTable(tableId)) {
      $(tableId).DataTable().destroy();
      $(tableId).empty(); 
    }
    ...
    // Tambahkan kembali thead karena $(tableId).empty() menghapusnya
    $(tableId).html(`
      <thead>
        <tr class="bg-slate-50 text-slate-600 uppercase text-[12px] font-bold tracking-widest border-b border-slate-200">
          <th class="p-4 text-center">Sumber</th>
          <th class="p-4 text-center">Tanggal</th>
          <th class="p-4 text-center">Nominal</th>
          <th class="p-4 text-center">Detail</th>
          ${userRole !== "Admin" ? '<th class="p-4 text-center">Aksi</th>' : ''}
        </tr>
      </thead>
    ...
    const dataFormatted = dataToDisplay.map(item => {
      ...
      if (userRole !== "Admin") row.push(item); 
      return row;
    });
    ...
    tableRef.current = $(tableId).DataTable({
      ...
      columns: [
        ...
        ...(userRole !== "Admin" ? [{
          ...
          render: (data) => `
            <div class="flex justify-center items-center gap-3">
              <button data-id="${data.id_unik}" data-modul="${data.modul_sumber}" class="btn-restore px-4 py-2 rounded bg-[#006400] text-white text-xs font-bold">Restore</button>
              <button data-id="${data.id_unik}" data-modul="${data.modul_sumber}" class="btn-delete px-4 py-2 rounded bg-rose-500 text-white text-xs font-bold">Hapus</button>
            </div>`
        }] : [])
      ],
      scrollX: true,
    ...
    // Delegasi Event Klik (Lebih Stabil)
    $(tableId).on('click', '.btn-restore', function() {
      const id = $(this).data('id');
      const modul = $(this).data('modul');
      const item = trashData.find(i => String(i.id_unik) === String(id) && i.modul_sumber === modul);
      if (item) { setConfirmData({ item, actionType: "restore" }); setConfirmOpen(true); }
    });
    ...
    $(tableId).on('click', '.btn-delete', function() {
      ...
      const modul = $(this).data('modul');
      const item = trashData.find(i => String(i.id_unik) === String(id) && i.modul_sumber === modul);
      if (item) { setConfirmData({ item, actionType: "delete" }); setConfirmOpen(true); }
    });
  ...
  const confirmYes = async () => {
    ...
    try {
      const res = await fetch(endpoint, {
        ...
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ [config.idKey]: item.id_unik }),
      });
      ...
      if (res.ok) {
        ...
        if (actionType === "restore") {
          if (item.modul_sumber === "DFOD") await fetchDfod();
          else if (item.modul_sumber === "DeliveryFee") await fetchDeliveryFee();
          else if (item.modul_sumber === "Outgoing") await fetchOutgoing();
          else if (item.modul_sumber === "Pengeluaran") await fetchPengeluaran();
          else if (item.modul_sumber === "Karyawan") await fetchKaryawan();
          await fetchTodayInputs();
          await fetchLaporanKeuangan();
        }
        setPopupMessage(`${item.modul_sumber} berhasil di${actionType === "restore" ? "kembalikan" : "hapus permanen"}`);
  ...
  // UI Render (Akses Ditolak & Loading)
  if (isCheckingRole) return <div className="p-10 text-center">Memverifikasi akses...</div>;
  if (userRole !== "Super Admin") {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-white p-6 text-center">
        <h2 className="text-2xl font-bold text-red-600">Akses Ditolak</h2>
        <p className="text-slate-500 mt-2">Hanya Super Admin yang dapat mengakses menu Trash.</p>
        <button onClick={() => window.location.href = "/dashboard"} className="mt-6 px-6 py-2 bg-[#006400] text-white rounded-lg">Kembali</button>
      </div>
    );
  }
  ...
  return (
    <main className="flex-1 bg-[#F8FAFC] p-6 min-h-screen">
      <div className="max-w-full">
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border border-slate-100 mb-8">
          <label className="block text-sm font-semibold text-slate-600 mb-1">Filter Sumber Data</label>
          <select
            className="p-2.5 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-[#006400] min-w-[200px]"
            value={filterModul}
            onChange={(e) => setFilterModul(e.target.value)}
          >
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border border-slate-100">
          <h3 className="text-xl font-bold mb-6 text-slate-800">Log Data Terhapus ({filterModul})</h3>
          <div className="overflow-x-auto">
            <table id="trashTable" className="w-full border-collapse display nowrap"></table>
          </div>
        </div>
      </div>
      ...
      {confirmOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-8 w-full max-w-sm text-center">
            ...
            <p className="text-slate-500 mb-6">Yakin ingin <strong>{confirmData?.actionType === "restore" ? "Restore" : "Hapus Permanen"}</strong> data ini?</p>
            <div className="flex gap-3 justify-center">
              <ButtonModular variant="warning" onClick={() => setConfirmOpen(false)}>Batal</ButtonModular>
              <ButtonModular variant={confirmData?.actionType === "restore" ? "success" : "danger"} onClick={confirmYes}>Ya, Lanjutkan</ButtonModular>
            </div>
          </div>
        </div>
      )}
